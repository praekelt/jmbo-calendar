...
class EntrySaveHandlersTestCase(unittest.TestCase):
    ...
    def test_save_handler_does_not_repeat(self):
        ...
        self.failUnlessRaises(
            Exception,
            models.save_handler_does_not_repeat,
            entry
        )
        ...
        self.failUnlessEqual(
            list(ent.calendars.all()),
            list(entry.calendars.all())
        )

        # Subsequent save should still only have a single
        # entryitem linked to this entry.
        models.save_handler_does_not_repeat(entry)
        entries = models.EntryItem.objects.filter(entry=entry)
        self.failUnlessEqual(entries.get().entry, entry)
    ...
    def test_save_handler_daily(self):
        ...
        # Should create an entryitem linked to this entry for each day
        # until entry's repeat until value.
        ...
        for ent in entries:
            ...
            # Created entry items should have same field
            # values as entry (except for repeating start and ends).
            self.failUnlessEqual(ent.content, entry.content)
            self.failUnlessEqual(
                list(ent.calendars.all()),
                list(entry.calendars.all())
        ...
        # Subsequent save should still only create an entryitem linked
        # to this entry for each day until entry's repeat until value.
        models.save_handler_daily(entry)
        entries = models.EntryItem.objects.filter(entry=entry)
        self.failUnlessEqual(entries.count(), 31)
    ...
    def test_save_handler_weekdays(self):
        ...
        entry = models.Entry(
            ...
            end=datetime(year=2000, month=1, day=1, hour=1, minute=1) + \
                    timedelta(days=1),
            repeat="weekdays",
            repeat_until=(datetime.now() + timedelta(days=30)).date(),
            content=self.content,
        ...
        # should create an entryitem linked to this entry for each
        # weekday until entry's repeat until value.
        entry.repeat_until = (entry.start + timedelta(days=30)).date()
        handler(entry)
        entries = models.EntryItem.objects.filter(entry=entry)
        ...
        for ent in entries:
            ...
            # Created entry items should have same field values as entry
            # (except for repeating start and ends).
            self.failUnlessEqual(ent.content, entry.content)
            self.failUnlessEqual(
                list(ent.calendars.all()),
                list(entry.calendars.all())
        ...
        # Subsequent save should still only create an entryitem linked to
        # this entry for each weekday until entry's repeat until value.
        handler(entry)
        entries = models.EntryItem.objects.filter(entry=entry)
        self.failUnlessEqual(entries.count(), 21)
    ...
    def test_save_handler_weekends(self):
        ...
        # Should create an entryitem linked to this entry for each
        # weekend day until entry's repeat until value.
        entry.repeat_until = (entry.start + timedelta(days=30)).date()
        handler(entry)
        entries = models.EntryItem.objects.filter(entry=entry)
    ...
    def test_save_handler_weekly(self):
        ...
        # should create an entryitem linked to this entry for each week until
        # entry's repeat until value, with the start day being the same
        # for each week.
        entry.repeat_until = (entry.start + timedelta(days=30)).date()
        handler(entry)
        entries = models.EntryItem.objects.filter(entry=entry)
        ...
        # Subsequent save should still only create an entryitem linked to this
        # entry for each weekday until entry's repeat until value.
        handler(entry)
        entries = models.EntryItem.objects.filter(entry=entry)
        self.failUnlessEqual(entries.count(), 5)
    ...
    def test_save_handler_monthly_by_day_of_month(self):
        ...
        entry = models.Entry(
            ...
            end=datetime(year=2000, month=2, day=1, hour=1, minute=1) + \
                    timedelta(days=1),
        ...
        # Should create an entryitem linked to this entry for each month until
        # entry's repeat until value, with the start day being the same day
        # date of the month for each month.
        ...
        # Subsequent save should still only create an entryitem linked to this
        # entry for each month until entry's repeat until value, with the
        # start day being the same day date of the month for each month.
        handler(entry)
        entries = models.EntryItem.objects.filter(entry=entry)
        self.failUnlessEqual(entries.count(), 7)
...
class PermittedManagerTestCase(unittest.TestCase):
    ...
    def test_get_query_set(self):
        ...
        # Entries with unpublished calendars and content should not be
        # available in queryset create unpublished cal and content entries.
        entry_obj = Entry(
            start=datetime.now(),
            end=datetime.now() + timedelta(days=1),
            repeat="daily",
            repeat_until=(datetime.now() + timedelta(days=30)).date(),
            content=unpublished_content
        )
        entry_obj.save()
        ...
        # Entries with unpublished calendars should not be available in
        # queryset, regardless of content state create unpublished cal,
        ...
        # Entries with unpublished content should not be available in
        # queryset, regardless of cal state create unpublished content,
        # published cal entries.
        entry_obj = Entry(
            start=datetime.now(),
            end=datetime.now() + timedelta(days=1),
            repeat="daily",
        ...
        # Entries with staging calendars and content should be available in
        # queryset but only if settings.STAGING = True create staging cal
        # and content entries.
        entry_obj = Entry(
            start=datetime.now(),
            end=datetime.now() + timedelta(days=1),
        ...
        # Entries with published cal and content should be available in
        # queryset create published cal and content entries.
        entry_obj = Entry(
            start=datetime.now(),
            end=datetime.now() + timedelta(days=1),
            repeat="daily",
        ...
        entry_obj = Entry(
            ...
            repeat="daily",
            repeat_until=(datetime.now() + timedelta(days=30)).date(),
            content=published_content_mobile
        )
        entry_obj.save()
    ...
    def test_by_range(self):
        ...
        end_contained_entryitem = EntryItem(
            entry_id=1,
            start=start - timedelta(days=1),
            end=end,
            content=content
        ...
        start_contained_entryitem = EntryItem(
            entry_id=1,
            start=start,
            end=end + timedelta(days=1),
            content=content
        ...
        # Entry starting before range but ending withing
        # range should be in result.
